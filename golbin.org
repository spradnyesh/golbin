...
*** [config] passing dimensions
    - let dimension be passed from URL as "?d1m=lang:en-IN,envt:dev", instead of "?lang=en-IN&envt=dev"
    - in restas/routes, if param contains d1m, then it should be passed on as it is in build-url
