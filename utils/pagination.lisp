         ...
         (with-html
           (:ol :class "pagination"
                (if (= ,page-number 0)
                         ...
                         (htm (:li (:a :href (genurl ,route ,@route-params :page 0) "first"))))
                (if (< (- ,page-number 10) 0)
                         (htm (:li :class "disabled" "-10"))
                         (htm (:li (:a :href (genurl ,route ,@route-params :page (- ,page-number 10)) "-10"))))
                (if (= ,page-number 0)
                         (htm (:li :class "disabled" "prev"))
                         (htm (:li (:a :href (genurl ,route ,@route-params :page (1- ,page-number)) "prev"))))
                ...
                (if (= ,page-number max-pages)
                         ...
                         (htm (:li (:a :href (genurl ,route ,@route-params :page (1+ ,page-number)) "next"))))
                (if (> (+ ,page-number 10) max-pages)
                         (htm (:li :class "disabled" "+10"))
                         (htm (:li (:a :href (genurl ,route ,@route-params :page (+ ,page-number 10)) "+10"))))
                (if (= ,page-number max-pages)
                         (htm (:li :class "disabled" "last"))
                         (htm (:li (:a :href (genurl ,route ,@route-params :page max-pages) "last")))))
